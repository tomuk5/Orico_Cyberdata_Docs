...
`bash cp /mnt/YOUR_POOL_NAME/Orico_Utils/systemd/led-daemon.service /etc/systemd/system/ && cp /mnt/YOUR_POOL_NAME/Orico_Utils/udev/99-orico-bays.rules /etc/udev/rules.d/ && udevadm control --reload-rules && udevadm trigger && systemctl daemon-reload && systemctl enable --now led-daemon.service`
